...
# html_files <- ("storage_05_obj_size-cdbdvlx11-ROOT-1.html")
...
for (i in html_files) {
  ...
  # sql 
  sqldata <- sqldf("select null pdb, null cdb, a.* from tbl_df a")
  sqldata$pdb <- pdb_name
  sqldata$cdb <- cdb_name
  
  # fix headers
  colnames(sqldata) <- c("pdb","cdb","RANK","SEGMENT_TYPE","OWNER","SEGMENT_NAME","OBJECT_ID","DATA_OBJECT_ID","TABLESPACE_NAME","TABLE_NAME","SEGMENTS","EXTENTS","BLOCKS","BYTES","GB","SEGMENT2","EXTENTS2","BLOCKS_2","BYTES_P","PERC_CU")
